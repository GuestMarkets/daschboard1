...
import {
  AlertCircle, Search, Users, Calendar as CalendarIcon, Clock, Repeat, Plus, Trash2, CheckCircle2, XCircle, AlertTriangle, ListChecks,
  TrendingUp, CalendarDays, AlertTriangle as AlertTri
} from "lucide-react";
...
function AssigneePicker({
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const data = await fetchJSON<{ items: UserLite[] }>(`/api/admin/users${q.trim() ? `?q=${encodeURIComponent(q.trim())}` : ""}`);
...
export default function DepartmentTasksPage() {
  ...
  useEffect(() => {
    ...
    // inclure 'sel' pour satisfaire react-hooks/exhaustive-deps
  }, [sel]);
  ...
  type KPI = { id: string; title: string; value: number; gradient: string; icon: IconType };
  ...
  async function createTask(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        recurrence: isRecurrent && freq !== "NONE" ? { frequency: freq, interval, count } : { frequency: "NONE" as const },
        created_by: currentUserId,
        department_id: departmentId,
      };
      const out = await fetchJSON<{ item: Task }>(`/api/admin/department/tasks`, { method: "POST", body: JSON.stringify(payload) });
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-3">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
            ...
            <select
              value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "all" | Status)}
            ...
            <select
              value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value as "all" | Priority)}
              className="h-9 px-2 rounded-lg text-sm ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 bg-white outline-none"
            >
            ...
            <select
              value={sort} onChange={(e) => setSort(e.target.value as "priority" | "date" | "name" | "progress")}
              className="h-9 px-2 rounded-lg text-sm ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 bg-white outline-none"
            >
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-3">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          ] as Array<readonly [Status, string, IconType]> ).map(([code, label, Icon]) => {
