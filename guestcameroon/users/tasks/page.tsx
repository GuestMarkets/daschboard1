...
type Task = {
  ...
  progress: number;
  performance: number;
  priority: Priority; // priorité stockée
...
async function fetchJSON<T = unknown>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    const err = isRecord(parsed) && typeof parsed.error === "string" ? parsed.error : `HTTP ${res.status}`;
    throw new Error(err);
  }
  return parsed as T;
}
...
/* ====== Priorité auto en fonction des heures écoulées ======
   Règle donnée :
    - À 50% des heures (création → échéance):
       * low → medium
       * medium → high
    - À 70% :
       * medium → high
   On élève au moins à ce niveau, on ne redescend jamais.
*/
function computeAutoPriority(
  base: Priority,
  createdAtISO: string,
  dueDate: string,
  dueTime?: string | null,
  ...
  if (totalMs <= 0) return "high"; // échéance dépassée → urgence
...
/* ===== AssigneePicker : recherche globale + cases à cocher ===== */
function AssigneePicker({
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const data = await fetchJSON<{ items: UserLite[] }>(
    ...
    <div className="rounded-lg ring-1 ring-slate-200 p-2">
      <div className="relative mb-2">
        ...
        <input
          ...
          className="w-full h-9 pl-7 pr-2 rounded-md text-sm ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none"
        />
      </div>
      <div className="max-h-52 overflow-auto">
        {loading && <div className="text-xs text-slate-500 px-1 py-1.5">Chargement…</div>}
        {!loading && items.length === 0 && <div className="text-xs text-slate-500 px-1 py-1.5">Aucun résultat</div>}
...
export default function DepartmentTasksPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await fetchJSON<{ items: Task[] }>(`/api/admin/department/tasks?department_id=${departmentId}`);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const data = await fetchJSON<{ items: Subtask[] }>(`/api/admin/department/tasks/${sel.id}/subtasks`);
  ...
  /* Résumé / KPIs (avec belles couleurs) */
  type KPI = { id: string; title: string; value: number; gradient: string; icon: LucideIcon };
  const kpis: KPI[] = useMemo(() => {
    ...
    return [
      ...
      { id: "k2", title: "En retard", value: overdue, gradient: "from-rose-500 to-orange-500", icon: AlertTri },
  ...
  const list = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      return (
        combineDateTime(a.due_date, a.due_time).getTime() - combineDateTime(b.due_date, b.due_time).getTime()
      );
    });
    return arr;
  ...
  async function createTask(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        recurrence: isRecurrent && freq !== "NONE" ? { frequency: freq, interval, count } : { frequency: "NONE" as RecurrenceFreq },
        created_by: currentUserId,
        department_id: departmentId,
      };
      const out = await fetchJSON<{ item: Task }>(`/api/admin/department/tasks`, {
        method: "POST",
  ...
  async function updateTask(patch: Partial<Task> & { id: number }) {
    ...
    try {
      const out = await fetchJSON<{ item: Task }>(`/api/admin/department/tasks/${patch.id}`, {
        method: "PATCH",
  ...
  /* Suppression directe (pas de pop-up) */
  ...
  async function addSubtask() {
    ...
    try {
      const r = await fetchJSON<{ item: Subtask }>(`/api/admin/department/tasks/${sel.id}/subtasks`, {
        method: "POST",
        body: JSON.stringify({ title: stTitle.trim(), description: stDesc.trim() || null }),
      });
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <form onSubmit={createTask} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-2.5">
              ...
              <div className="md:col-span-3">
                <label className="block text-[11px] text-slate-600 mb-1 flex items-center gap-1">
                  <Users className="w-3.5 h-3.5" /> Attribuer à (tous les utilisateurs)
                </label>
            ...
            <div className="flex items-center justify-end">
              <button
                ...
                className={cls(
                  "h-9 px-4 rounded-lg text-white text-sm flex items-center gap-2",
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-3">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
            ...
            <select
              value={sort}
              onChange={(e) => setSort(e.target.value as "priority" | "date" | "name" | "progress")}
              className="h-9 px-2 rounded-lg text-sm ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 bg-white outline-none"
            >
          ...
          <section className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-3">
            ...
            <div className="divide-y divide-slate-100">
              {list.map((t) => {
                ...
                // Libellé spécial pour "blocked" => "Débloquée"
                const statusText = t.status === "todo" ? "À faire" : t.status === "in_progress" ? "En cours" : t.status === "blocked" ? "Débloquée" : "Terminée";
                ...
                return (
                  <div
                    key={t.id}
                    className={cls(
                      "py-3 px-3 rounded-xl hover:bg-slate-50 transition-colors",
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[12px] text-slate-500 mt-1">
                          Assignés : {(t.assignees ?? []).length ? (t.assignees ?? []).map((a) => a.name).join(", ") : "—"} • Progression : {t.progress}% • Performance :{" "}
                          {t.performance}%
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setSel(t)}
                          className={cls(
                            "h-8 px-3 rounded-lg text-[12.5px] ring-1 transition-colors",
                        ...
                        <button
                          ...
                          className={cls(
                            "h-8 px-3 rounded-lg text-[12.5px] inline-flex items-center gap-1 transition-colors",
...
function TaskDetails({
  ...
  const statusButtons: Array<[Status, string, LucideIcon]> = [
    ["todo", "À faire", Clock],
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-3">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <div className="text-[11px] text-slate-600 mb-1">Priorité (affichée : automatique selon l’échéance)</div>
            <select
              value={sel.priority}
              onChange={(e) => onChange({ ...sel, priority: e.target.value as Priority })}
          ...
          <div className="md:col-span-2">
            <div className="text-[11px] text-slate-600 mb-1">Assignés (tous les utilisateurs)</div>
            <AssigneePicker selectedIds={sel.assigneeIds} onChange={(ids) => onChange({ ...sel, assigneeIds: ids })} />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) => onChange({ ...sel, description: e.target.value })}
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                className="accent-blue-600 scale-110"
                checked={sel.is_recurrent}
                onChange={(e) => onChange({ ...sel, is_recurrent: e.target.checked })}
              />
        ...
        {/* Boutons statut (NB: 'blocked' s’affiche comme “Débloquée” en liste) */}
        <div className="grid grid-cols-2 gap-2">
          {statusButtons.map(([code, label, IconC]) => {
            const active = sel.status === code;
            const base =
              code === "done"
                ? "bg-green-100 text-green-700 ring-green-300 hover:bg-green-200"
                : code === "blocked"
            ...
            return (
              <button
                key={code}
                disabled={disabled}
                onClick={() => onSave({ ...sel, status: code })}
                className={cls(
                  "px-3 py-2.5 rounded-lg text-sm font-medium ring-1 transition-all flex items-center gap-2 justify-center",
        ...
        <div className="flex justify-end">
          <button onClick={() => onSave(sel)} className="h-9 px-4 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm">
            Enregistrer
          </button>
        </div>
        ...
        <div className="pt-3 border-t border-slate-200">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {subtasks.map((s) => (
              <div key={s.id} className="p-2 rounded-lg ring-1 ring-slate-200 hover:ring-slate-300 transition-colors">
                <label className="flex items-start gap-3">
                  <input
                    type="checkbox"
                    className="mt-1 accent-blue-600 scale-110"
                    checked={!!s.done}
                    onChange={() => onToggleSubtask(s)}
                  />
                  <div className="min-w-0 flex-1">
                    <div className={`text-sm break-words font-medium ${s.done ? "line-through text-slate-400" : "text-slate-800"}`}>{s.title}</div>
            ...
            {subtasks.length === 0 && (
              <div className="text-sm text-slate-500 text-center py-4 border border-dashed border-slate-200 rounded-lg">Aucune sous-tâche</div>
            )}
          </div>
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-5 gap-2">
            ...
            <input
              ...
              className="h-9 sm:col-span-2 px-3 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            />
            <button onClick={onAddSubtask} className="h-9 px-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm flex items-center justify-center gap-1">
