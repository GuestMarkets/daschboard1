...
/* --------------------------------- API row --------------------------------
 * Ce type couvre camelCase et snake_case tels que renvoyés par l'API.
 */
type ApiProjectRow = {
  id: number | string;
  name?: string | null;
...
function errorMessage(e: unknown): string {
  ...
  try { return String(e); } catch { return "Erreur"; }
}
...
function computeProgressAuto(startDate: string, endDate: string): number {
  const start = atLocal(startDate, "08:00");
  const end   = atLocal(endDate, "17:00");
  const now = new Date();
...
function CreateProjectModal({
  ...
  const [, setSaving] = useState(false); // variable non lue, on ne nomme pas l'état
  const [err, setErr] = useState<string | null>(null);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(errorMessage(e) || "Erreur inconnue");
    } finally {
      setSaving(false);
    }
  }
    ...
    <Modal open={open} onClose={onClose} title="Créer un projet" size="lg">
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-end gap-2 pt-2">
          ...
          <button className="px-4 h-10 rounded-xl text-white font-semibold bg-indigo-600 hover:bg-indigo-700">
            Créer le projet
          </button>
        </div>
      </form>
...
function EditProjectModal({
  ...
  }, [startDate, endDate]); // on force uniquement sur les dates
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const guardedAssignees = assigneeIds.includes(me!.id) ? assigneeIds : [me!.id, ...assigneeIds];
      const patch = {
      ...
      {!project ? null : (
        <form onSubmit={submit} className={cls("space-y-3", locked && "opacity-70 pointer-events-none")}>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-end gap-2 pt-2">
            ...
            {!locked && <button className="px-4 h-10 rounded-xl text-white bg-indigo-600 hover:bg-indigo-700">Enregistrer</button>}
          </div>
        </form>
      )}
    </Modal>
...
async function fetchJSON<T = unknown>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (ct.includes("application/json")) { try { data = await res.json(); } catch { data = null; } }
    ...
    catch { data = { error: (text || "").replace(/\s+/g, " ").slice(0, 200) || "Réponse non-JSON" } as { error: string }; }
  }
  if (!res.ok) {
...
export default function ProjectsPage() {
    ...
    (async () => {
      try {
        ...
        const [meRes, usersRes, projRes] = await Promise.all([
          ...
          fetchJSON<{ projects: ApiProjectRow[] }>(`/guestmarkets/api/projects/assigned`),
        ]);
        setMe(meRes.me || null);
  ...
  const visualItems = useMemo(() => {
    // on "utilise" clock pour justifier la dépendance et recalculer périodiquement
    void clock;
    return items.map(p => {
      const autoProg = computeProgressAuto(p.startDate, p.endDate);
  ...
  function normalizeProject(row: ApiProjectRow): Project {
    const updatedRaw = row.updatedAt ?? row.updated_at ?? null;
    const updatedAt = (updatedRaw && !Number.isNaN(new Date(updatedRaw).getTime()))
    ...
    const manager =
      ...
      (row.manager_id && row.manager_name
        ? { id: Number(row.manager_id), name: String(row.manager_name) }
        : null);
    ...
    return {
      ...
      managerId: typeof row.managerId === "number" ? row.managerId : (row.manager_id ?? row.manager?.id ?? null),
      manager,
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 mt-3 mb-3">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <select
              value={sort}
              onChange={(e)=>setSort(e.target.value as ("date" | "name" | "progress"))}
              className="text-gray-700 h-9 px-2 rounded-lg text-sm ring-1 ring-slate-200 bg-white focus:ring-2 focus:ring-indigo-500 outline-none"
            >
                ...
                <button
                  onClick={async ()=>{
                    ...
                    try {
                      ...
                      // Silencieux : on pourrait mettre un toast
                    }
                  }}
