...
// petit helper de garde de type pour { error?: string }
function hasErrorMessage(x: unknown): x is { error?: string } {
...
async function fetchJSON<T = unknown>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    const msg = hasErrorMessage(data) && typeof data.error === "string" ? data.error : `HTTP ${res.status}`;
    throw new Error(msg);
  }
  return data as T;
}
...
function SoloUI() {
  ...
  // (Suppression modale non utilisée : delOf / setDelOf supprimés)
  ...
  useEffect(() => {
    ...
    const unsub = rt.subscribe((a: { type: string }) => { if (a.type === "meetings_changed") refresh(); });
    return unsub;
  }, [rt, refresh]);
