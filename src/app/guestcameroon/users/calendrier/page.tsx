...
async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    // ignore localStorage access errors (SSR/blocked)
  }
  ...
  if (!res.ok) {
    const message = isApiError(data) && data.error ? data.error : `HTTP ${res.status}`;
...
function EventModal({
  ...
  const [startAt, setStartAt] = useState<string>(
    defaults?.startAt || toLocalInput(nowPlus1h)
  );
  const [endAt, setEndAt] = useState<string>(
    defaults?.endAt || toLocalInput(new Date(nowPlus1h.getTime() + 60 * 60 * 1000))
  );
  const [attendees, setAttendees] = useState<number[]>(
    defaults?.attendees?.map((a) => a.id) || []
  );
  const [taskId, setTaskId] = useState<number | "">(
    ((defaults?.taskId || "") as unknown) as number | ""
  );
  ...
  useEffect(() => {
    ...
    setEndAt(
      defaults?.endAt ||
        toLocalInput(new Date(nowPlus1h.getTime() + 60 * 60 * 1000))
    );
    setAttendees(defaults?.attendees?.map((a) => a.id) || []);
    setTaskId(((defaults?.taskId || "") as unknown) as number | "");
    setErr(null);
    setSaving(false);
    setSyncGoogle(false);
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={defaults?.id ? "Modifier l’événement" : "Nouvel événement"}
      size="lg"
    >
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className="w-full min-h-[70px] px-3 py-2 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none resize-y text-sm"
            />
          </div>

          <div>
            <label className="text-[12px] text-slate-600 mb-1 block flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="chk-allday" className="text-sm text-slate-700">
              Toute la journée
            </label>
          </div>
          ...
          <div>
            ...
            <div className="text-[11px] text-slate-500 mt-1">
              Ctrl/⌘ pour multi-sélection.
            </div>
          </div>

          <div>
            <label className="text-[12px] text-slate-600 mb-1 block flex items-center gap-1">
              <Link2 className="w-3.5 h-3.5" /> Lier à une tâche
            </label>
            <select
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            ...
            <label
              ...
              className={cls(
                "text-sm",
                googleConnected ? "text-slate-700" : "text-slate-400"
              )}
            >
              Synchroniser avec mon Google Agenda{" "}
        ...
        <div className="flex justify-end gap-2 pt-1">
          <button
            type="button"
            onClick={onClose}
            className="px-3 h-9 rounded-lg ring-1 ring-slate-200 hover:bg-slate-50 text-sm"
          >
            Annuler
          </button>
          <button
...
export default function CalendarPage() {
    ...
    (async () => {
      try {
        const [u, t, g] = await Promise.all([
          fetchJSON<{ items: UserLite[] }>("/api/calendar/users-lite"),
          fetchJSON<{ items: TaskLite[] }>("/api/calendar/tasks-lite"),
          fetchJSON<{ connected: boolean }>("/api/calendar/google/status"),
        ]);
        setUsers(u.items || []);
        setTasks(t.items || []);
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await fetchJSON<{ items: EventItem[] }>(
  ...
  function eventsOnDay(d: Date) {
    ...
    return events
      .filter((ev) => {
        ...
        const inRange =
          new Date(y, m, dd) <= e &&
          new Date(y, m, dd, 23, 59, 59, 999) >= s;
        return inRange;
      })
      .slice(0, 4); // limiter l’affichage
  }

  function openCreateOn(day: Date) {
    const start = new Date(day);
    start.setHours(Math.max(9, today.getHours()), 0, 0, 0);
  ...
  async function handleSubmit(payload: EventPayload) {
    if (editing) {
      const data = await fetchJSON<{ item: EventItem }>(
      ...
      const data = await fetchJSON<{ item: EventItem }>(`/api/calendar/events`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    ...
    <>
      ...
      <div className="lg:pl-64 min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 pt-14 md:pt-[72px]">
        ...
        <main className="max-w-7xl mx-auto px-4 py-5 space-y-4">
          ...
          >
            <div className="flex items-center gap-2">
              <CalendarIcon
                className={cls(
                  "w-4 h-4",
                  googleConnected ? "text-blue-700" : "text-slate-600"
                )}
              />
              <div className="text-sm">
                {googleConnected
                  ? "Connecté à Google Agenda. Vous pouvez synchroniser vos événements à la création/modification."
                  : "Vous pouvez connecter votre Google Agenda pour synchroniser vos événements."}
              </div>
              <div className="ml-auto flex items-center gap-2">
                {!googleConnected ? (
                  <a
                    href="/api/calendar/google/auth-url"
                    onClick={async (e) => {
                      e.preventDefault();
                      try {
                  ...
                  <button
                    onClick={async () => {
                      try {
                        await fetchJSON<unknown>("/api/calendar/google/disconnect", {
                          method: "POST",
                        });
          ...
          {/* Affichage erreur globale si besoin */}
          {err && (
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="h-9 px-3 rounded-lg ring-1 ring-slate-200 bg-white hover:bg-slate-50 text-sm"
              >
                Aujourd’hui
              </button>
            </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
            <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50/60">
              {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((d) => (
                <div key={d} className="px-3 py-2 text-[12px] text-slate-600">
                  {d}
                </div>
              ))}
            </div>
              ...
              {days.map((d, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={cls(
                      "min-h[110px] min-h-[110px] border-b border-r border-slate-100 p-2 hover:bg-slate-50 transition",
                      idx % 7 === 6 ? "!border-r-0" : "",
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div
                        className={cls(
                          "text-[12px]",
                          isToday ? "text-blue-700 font-semibold" : "text-slate-600"
                        )}
                      >
                        {fmtDay(d)}
                      </div>
