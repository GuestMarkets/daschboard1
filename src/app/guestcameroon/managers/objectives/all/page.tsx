...
import {
  ...
  Calendar as CalendarIcon, Clock, TrendingUp, UserPlus
} from "lucide-react";
...
/** Représentation brute possible en sortie d'API (clé snake_case / camelCase) */
type ApiSubtask = {
  id: number;
  objective_id?: number;
  title?: string;
...
async function fetchJSON<T = unknown>(url: string, init?: RequestInit): Promise<T> {
  ...
  try { const t = localStorage.getItem("auth_token"); if (t && !headers.has("Authorization")) headers.set("Authorization", `Bearer ${t}`); } catch {}
  const res = await fetch(url, { credentials: "include", ...init, headers });
  const raw = await res.text();
  let data: unknown = {};
  try { data = raw ? JSON.parse(raw) : {}; } catch { data = { error: raw?.slice(0,200) || "Réponse non-JSON" }; }
  if (!res.ok) {
...
/** ===== Normalisation API → UI (module scope pour éviter warnings deps) ===== */
function normalize(o: ApiObjective): Objective {
  return {
    id: Number(o.id),
    userId: Number(o.userId ?? o.user_id ?? o.owner_id ?? 0),
    owner: String(o.owner ?? o.owner_name ?? o.user_name ?? ""),
    ...
    subtasks: Array.isArray(o.subtasks) ? o.subtasks.map((s: ApiSubtask) => ({
      id: Number(s.id),
      objective_id: Number(s.objective_id ?? o.id),
      title: String(s.title ?? ""),
      weight: Number(s.weight ?? 0),
      dueDate: s.dueDate ?? s.due_date ?? null,
      done: s.done ? 1 : 0
    })) : [],
    calendar_event_id: o.calendar_event_id ?? null,
  };
}
function normalizeList(arr: ApiObjective[]) { return arr.map(normalize); }
...
export default function ObjectivesAllPage() {
    ...
    (async()=>{
      try {
        ...
        const [u, o] = await Promise.all([
          fetchJSON<ApiList<UserLite>>("/api/users?lite=1&pageSize=500"),
          fetchJSON<{ items: ApiObjective[] }>("/api/objectives?scope=all"),
        ]);
        setUsers(u.items || []);
  ...
  async function createObjective(e: React.FormEvent) {
    ...
    try {
      ...
      const out = await fetchJSON<{ item: ApiObjective }>("/api/objectives", { method: "POST", body: JSON.stringify(payload) });
  ...
  async function toggleSub(o: Objective, s: SubObj) {
    const r = await fetchJSON<{ item: ApiObjective }>(`/api/objectives/${o.id}/subtasks/${s.id}`, {
  ...
  async function completeObjective(o: Objective) {
    const r = await fetchJSON<{ item: ApiObjective }>(`/api/objectives/${o.id}`, {
  ...
  const [assignMap, setAssignMap] = useState<Record<number, number>>({}); // objectiveId -> userId (sélection)
  ...
  async function applyAssign(obj: Objective) {
    ...
    try {
      const r = await fetchJSON<{ item: ApiObjective }>(`/api/objectives/${obj.id}`, {
        method: "PATCH",
  ...
  async function saveSubsOnly(
    ...
    const r = await fetchJSON<{ item: ApiObjective }>(`/api/objectives/${id}`, {
  ...
  async function addNote(id:number, text:string) {
    // pas de payload typé en retour nécessaire
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            ...
            const Icon: React.ComponentType<React.SVGProps<SVGSVGElement>> = k.icon;
            return (
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {!loading && list.map(o => {
            ...
            const prBadge = pr==="high" ? "bg-red-100 text-red-700 ring-red-200"
...
const Bar: React.FC<{ value:number }> = ({ value }) => (
  <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden">
    <div className="h-full bg-gradient-to-r from-indigo-600 to-fuchsia-600" style={{ width: `${Math.max(0, Math.min(100, value))}%` }} />
  </div>
);
