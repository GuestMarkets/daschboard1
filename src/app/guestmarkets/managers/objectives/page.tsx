...
import {
  ...
  Calendar as CalendarIcon, Clock, TrendingUp, Pencil
} from "lucide-react";
...
/** Structures “raw” reçues depuis l’API */
type RawSubtask = {
  id?: number | string;
...
type ApiMe = { ok: boolean; user?: { id: number; name?: string } };
type ApiObjectives = { items: RawObjective[] };
type ApiObjectiveItem = { item: RawObjective };
...
async function fetchJSON<T = unknown>(url: string, init?: RequestInit): Promise<T> {
  ...
  let data: unknown = {};
  try { data = txt ? JSON.parse(txt) : {}; } catch { data = { error: txt?.slice(0, 200) || "Réponse non-JSON" }; }
  if (!res.ok) {
    const maybeObj = data as { error?: string };
    throw new Error(maybeObj?.error || `HTTP ${res.status}`);
  }
  return data as T;
}
...
/** Typage pour icônes Lucide */
type IconComponent = React.ComponentType<React.SVGProps<SVGSVGElement>>;

/* =================== Normalisation =================== */
function normalizeSubtask(oId: number, s: RawSubtask): SubObj {
  return {
    id: Number(s.id ?? 0),
    objective_id: Number(s.objective_id ?? oId),
...
function normalize(o: RawObjective): Objective {
  const start = String(o.startDate ?? o.start_date ?? todayStr());
  const end = String(o.endDate ?? o.end_date ?? todayStr());
  const id = Number(o.id);
  const subtasks: SubObj[] = Array.isArray(o.subtasks)
    ? o.subtasks.map((s) => normalizeSubtask(id, s))
    : [];
  return {
    id,
...
export default function MyObjectivesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const me = await fetchJSON<ApiMe>("/api/me");
        const uid = me?.user?.id ?? null;
        const uname = (me?.user?.name || "").trim();
        setMeId(uid);
        setMeName(uname);

        const o = await fetchJSON<ApiObjectives>("/api/objectives?scope=my");
  ...
  const list = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const pa = ord[displayPriority(a)];
      const pb = ord[displayPriority(b)];
      if (pa !== pb) return pb - pa;
      return safeDate(a.endDate).getTime() - safeDate(b.endDate).getTime();
    });
    return arr;
  }, [items, q, statusFilter, sort, displayPriority]);
  ...
  async function createObjective(e: React.FormEvent) {
    ...
    try {
      ...
      const out = await fetchJSON<ApiObjectiveItem>("/api/objectives", {
        method: "POST",
  ...
  async function toggleSub(o: Objective, s: SubObj) {
    const r = await fetchJSON<ApiObjectiveItem>(`/api/objectives/${o.id}/subtasks/${s.id}`, {
      method: "PATCH",
  ...
  async function completeObjective(o: Objective) {
    const r = await fetchJSON<ApiObjectiveItem>(`/api/objectives/${o.id}`, {
      method: "PATCH",
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            ...
            const Icon: IconComponent = k.icon as IconComponent;
            return (
            ...
            <div className="md:col-span-2 xl:col-span-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-gray-700 inline-flex items-center gap-1 px-3 h-9 rounded-lg bg-slate-900 text-white hover:bg-slate-800"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                {subs.map((s, i) => (
                  <div key={i} className="col-span-12 grid grid-cols-12 gap-2">
                    <input
                      className="text-gray-700 col-span-6 h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-400 outline-none"
                    ...
                    <input
                      ...
                      min={0}
                      max={100}
                      className="text-gray-700 col-span-2 h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-400 outline-none"
                      placeholder="Poids %"
                      value={s.weight}
                    ...
                    <input
                      type="date"
                      className="text-gray-700 col-span-3 h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-400 outline-none"
                      value={s.dueDate}
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="text-gray-700 h-10 pl-7 pr-2 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-400 outline-none text-sm"
                />
              </div>
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as ObjStatus | "all")}
                className="text-gray-700 h-10 px-3 rounded-xl ring-1 ring-slate-200 bg-white focus:ring-2 focus:ring-blue-400 outline-none text-sm"
              >
              ...
              <select
                value={sort}
                onChange={(e) => setSort(e.target.value as "priority" | "end" | "progress")}
                className="text-gray-700 h-10 px-3 rounded-xl ring-1 ring-slate-200 bg-white focus:ring-2 focus:ring-blue-400 outline-none text-sm"
              >
...
function EditObjectiveModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const r = await fetchJSON<ApiObjectiveItem>(`/api/objectives/${local.id}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Modifier l’objectif" size="lg">
      {/* children requis par Modal → on envoie toujours du contenu */}
      {!local ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="text-gray-700 h-9 w-full px-3 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
              />
            </div>
            <div>
              <div className="text-[12px] text-slate-600 mb-1">Unité</div>
            ...
            <div>
              <div className="text-gray-700 text-[12px] text-slate-600 mb-1">Cible</div>
              <input
                type="number"
                min={0}
                value={local.target}
