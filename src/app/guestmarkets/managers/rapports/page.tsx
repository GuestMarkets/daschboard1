...
/** Rôles métier */
type UserRole = "user" | "projectManager" | "departmentManager" | "superAdmin";
...
/** On rend le type plus souple, car le backend peut renvoyer d’autres tags */
type CandidateTag = "superAdmin" | "projectManager" | "departmentManager" | string;
      ...
      <div className="mt-4 grid grid-cols-2 sm:grid-cols-6 gap-2">
        {(
          [
            ...
            [
              "Rejetés / Corr.",
              (stats.rejected ?? 0) + (stats.changes_requested ?? 0),
            ],
          ] as const
        ).map(([label, val]) => (
          <div
...
function RecipientPicker({
  ...
  // Style par défaut si le tag n’est pas connu (ex : "user")
  ...
  const roleLabel =
    currentUser?.role === "user"
      ? "En tant qu’utilisateur, vous pouvez envoyer un rapport uniquement à vos responsables (équipe / projet / département) et aux super administrateurs."
      : "Vous pouvez sélectionner librement les destinataires autorisés pour ce rapport.";

  return (
    <div className="rounded-xl ring-1 ring-slate-200 p-2 bg-white/80">
      <div className="text-[12px] text-slate-600 mb-1 flex justify-between items-center">
        <span>Envoyer à</span>
      ...
      <div className="flex flex-wrap gap-1.5 mt-1">
        {candidates.length === 0 ? (
          ...
          candidates.map((c) => {
            const meta = tags[c.tag] ?? defaultTagMeta;
            ...
            return (
              <button
                ...
                onClick={() => toggle(c.id)}
                className={cls(
                  "inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] ring-1 transition",
                  tone,
                  active ? "ring-2 ring-offset-1 ring-offset-white" : "opacity-80"
                )}
                title={meta.label}
              >
      ...
      <div className="text-[11px] text-slate-500 mt-1">
        {value.length} destinataire(s) sélectionné(s)
      </div>
    </div>
  );
}
...
function ReportCreateModal({
      ...
      try {
        if (!isBasicUser) {
          const [p, d] = await Promise.all([
            fetchJSON<ListResponse<ProjectLite>>(
              "/guestmarkets/api/projects/lite"
            ).catch(() => ({ items: [] })),
            fetchJSON<ListResponse<DepartmentLite>>(
              "/guestmarkets/api/departments/lite"
            ).catch(() => ({ items: [] })),
          ]);
          setProjects(p.items || []);
          setDepts(d.items || []);
          ...
          setProjects(
            currentUser?.projectId
              ? [{ id: currentUser.projectId, name: "Votre projet" }]
              : []
          );
          setDepts(
            currentUser?.departmentId
              ? [{ id: currentUser.departmentId, name: "Votre département" }]
              : []
          );
        }
      } catch {
  ...
  }, [open, isBasicUser, currentUser?.projectId, currentUser?.departmentId]);

  useEffect(() => {
    if (!open) return;
    if (isBasicUser) {
      setProjectId(
        currentUser?.projectId != null ? Number(currentUser.projectId) : ""
      );
      setDepartmentId(
        currentUser?.departmentId != null ? Number(currentUser.departmentId) : ""
      );
    } else {
      setProjectId("");
      setDepartmentId("");
  ...
  const refreshCandidates = useCallback(
    async (pid: number | "", did: number | "") => {
      try {
        ...
        const resp = await fetchJSON<CandidatesResponse>(
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (type === "weekly" && !validWeekly(periodStart, periodEnd))
      return setErr(
        "Hebdomadaire : exactement 7 jours et pas au-delà d’aujourd’hui."
      );
    if (type === "monthly" && !validMonthly(periodStart, periodEnd))
      return setErr(
        "Mensuel : exactement le mois sélectionné (30/31 jours) et pas au-delà d’aujourd’hui."
      );
    if (recipientIds.length === 0)
      return setErr("Sélectionnez au moins un destinataire.");
  ...
  return (
    <Modal open={open} onClose={onClose} title="Nouveau rapport" size="lg">
      <form onSubmit={submit} className="space-y-3">
        {err && (
          <div className="text-sm px-3 py-2 rounded-lg border border-red-200 bg-red-50 text-red-700 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              className="w-full h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-slate-800 bg-white"
            />
          </div>
          <div>
            ...
            <>
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm bg-white"
                />
              </div>
              <div>
                <label className="block text-[12px] text-slate-600 mb-1">
          ...
          <div>
            ...
            <select
              ...
              className={cls(
                "w-full h-10 px-3 rounded-xl ring-1 ring-slate-200 bg-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-slate-800",
                isBasicUser && "bg-slate-50 text-slate-500 cursor-not-allowed"
              )}
            >
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className="w-full min-h-[90px] px-3 py-2 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none resize-y text-sm text-slate-800 bg-white"
            />
          </div>
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 h-10 rounded-xl ring-1 ring-slate-200 hover:bg-slate-50 text-slate-700 text-sm"
          >
            Annuler
          </button>
          <button
            disabled={saving}
            className={cls(
              "px-4 h-10 rounded-xl text-white font-semibold text-sm",
              saving
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="w-full h-10 px-3 rounded-xl ring-1 ring-slate-200 focus:ring-2 outline-none text-sm bg-white"
              />
            </div>
            <div className="grid content-end text-[12px] text-slate-600">
              Statut : <b>{r.status}</b>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                value={summary ?? ""}
                onChange={(e) => setSummary(e.target.value)}
                className="w-full min-h-[90px] px-3 py-2 rounded-xl ring-1 ring-slate-200 focus:ring-2 outline-none resize-y text-sm bg-white"
              />
            </div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 h-10 rounded-xl ring-1 ring-slate-200 hover:bg-slate-50 text-sm"
            >
              Fermer
            </button>
            {!disabled && (
              <button
                disabled={saving}
                className={cls(
                  "px-4 h-10 rounded-xl text-white text-sm",
                  saving
                    ? "bg-slate-400 cursor-not-allowed"
                    : "bg-indigo-600 hover:bg-indigo-700"
                )}
...
function ReportDetailModal({
  ...
  const canDecide =
    ...
    currentUserRole === "departmentManager";
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setLoading(true);
        setErr(null);
        const { item } = await fetchJSON<ReportDetailResponse>(
    ...
    <Modal open={open} onClose={onClose} title="Détails du rapport" size="lg">
      {!item ? (
        ...
        <div className="space-y-3">
              ...
              {item.files.length ? (
                item.files.map((f) => (
                  <a
                    ...
                    rel="noreferrer"
                  >
                    <FileText className="text-slate-700 w-3.5 h-3.5" />{" "}
                    {f.original_name}
                  </a>
                ))
              ) : (
          ...
          <div className="rounded-xl bg-white ring-1 ring-slate-200 p-3">
            <label className="block text-[12px] text-slate-600 mb-1 flex items-center gap-1.5">
              <ClipboardEdit className="w-4 h-4" /> Motif / corrections (si
              pertinent)
            </label>
            <textarea
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              className="w-full min-h-[80px] px-3 py-2 rounded-xl ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none resize-y text-sm bg-white"
            />
            {!canDecide && (
              <p className="text-[11px] text-slate-500 mt-1">
                Vous pouvez consulter le rapport, mais seules les personnes
                responsables (chef de projet / chef de département / super
                admin) peuvent prendre une décision.
              </p>
            )}
          </div>
            ...
            {canDecide ? (
              <>
                <button
                  ...
                  className={cls(
                    "px-3 h-9 rounded-lg text-white text-sm",
                    acting === "changes_requested"
              ...
              <button
                onClick={onClose}
                className="px-3 h-9 rounded-lg text-sm bg-slate-800 text-white hover:bg-slate-900"
              >
                Fermer
              </button>
            )}
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const u = await fetchJSON<CurrentUserResponse>("/guestmarkets/api/me");
        setCurrentUser(u);
        setUserErr(null);
      } catch (e: unknown) {
        setUserErr(
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [list, st] = await Promise.all([
        fetchJSON<ListResponse<Report>>(
          `/guestmarkets/api/reports?scope=${scope}`
        ),
        fetchJSON<StatsResponse>(
          `/guestmarkets/api/reports/stats?scope=${scope}`
        ),
      ]);
      setItems(list.items || []);
      setStats(st || {});
  ...
  async function uploadPdf(reportId: number, file: File) {
    ...
    fd.set("file", file);
    const data = await fetchJSON<UploadedFileResponse>(
      `/guestmarkets/api/reports/${reportId}/files`,
      { method: "POST", body: fd }
    );
    setItems((prev) =>
      prev.map((r) =>
        r.id === reportId ? { ...r, files: [data.file, ...(r.files || [])] } : r
      )
    );
  }
  ...
  async function actOnReport(
    ...
    await fetchJSON<unknown>(
      `/guestmarkets/api/reports/${detailId}/review`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <Shell sidebarTitle="Guest Office" sidebarSubtitle="Rapports">
      ...
      {userErr && (
        <div className="mb-3 text-xs px-3 py-2 rounded-lg border border-amber-200 bg-amber-50 text-amber-800 flex items-center gap-1.5">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-2">
          <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1 w-fit">
            <button
              ...
              className={cls(
                "px-3 h-9 rounded-lg text-sm transition",
                scope === "sent"
          ...
          <div className="relative ml-0 lg:ml-3">
            ...
            <input
              ...
              className="text-gray-700 h-9 pl-7 pr-2 rounded-lg ring-1 ring-slate-200 focus:ring-indigo-500 outline-none text-sm bg-white"
            />
          </div>
          ...
          <div className="flex gap-2 lg:ml-auto">
            ...
            <select
              value={typeFilter}
              onChange={(e) =>
                setTypeFilter(e.target.value as "all" | Report["type"])
              }
              className="text-gray-700 h-9 px-2 rounded-lg ring-1 ring-slate-200 bg-white text-sm"
            >
            ...
            <button
              ...
              className="h-9 px-3 rounded-lg bg-indigo-600 text-white text-sm hover:bg-indigo-700 flex items-center gap-1"
            >
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        {err && !loading && (
          <div className="col-span-full text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2 flex items-center gap-1.5">
        ...
        {!loading &&
          list.map((r, idx) => (
            <div
              key={r.id}
              className={cls(
                "rounded-2xl border border-slate-200 p-3 transition bg-white",
                scope === "received" && r.unread
            ...
            >
              <div className="flex items-start justify-between gap-2">
                  ...
                  {scope === "received" ? (
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {r.recipients.map((u, i) => {
                        const tone =
                          tonePool[(u.id + i + idx) % tonePool.length];
                        return (
                          <span
                            key={u.id}
                            className={cls(
                ...
                <span
                  className={cls(
                    ...
                    (scope === "received" ? r.inboxStatus : r.status) ===
                      "approved"
                      ? "bg-emerald-50 text-emerald-800 ring-emerald-200"
                      : (scope === "received" ? r.inboxStatus : r.status) ===
                        "rejected"
              ...
              {r.summary && (
                <div className="mt-2 text-[13px] text-slate-700 line-clamp-3">
                  {r.summary}
                </div>
              )}
                  ...
                  (r.status === "draft" ||
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button
                        ...
                        className="text-slate-500 h-8 px-2 rounded-lg ring-1 ring-slate-200 hover:bg-slate-50 text-[12.5px] inline-flex items-center gap-1 bg-white"
                      >
              ...
              <div className="mt-3 flex justify-end gap-1.5">
                {scope === "sent" ? (
                  ...
                  <button
                    onClick={() => setDetailId(r.id)}
                    className={cls(
                      "h-8 px-2.5 rounded-lg text-white text-[12.5px] inline-flex items-center gap-1",
