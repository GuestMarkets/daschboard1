...
export default function MyTasksPage() {
  // Utilisateur courant (id seulement)
  ...
  /* Profil (uniquement pour récupérer l'id) */
  useEffect(() => {
    (async () => {
      try {
        ...
        const msg =
          e instanceof Error ? e.message : "Erreur lors du chargement du profil.";
        setErr(msg);
      }
    })();
  }, []);
    ...
    (async () => {
      ...
      try {
        const data = await fetchJSON<{ items: Subtask[] }>(
          `/guestmarkets/api/tasks/${sel.id}/subtasks`
        );
  ...
  async function updateTask(patch: Partial<Task> & { id: number }) {
    try {
      const out = await fetchJSON<{ item: Task }>(
        `/guestmarkets/api/tasks/${patch.id}`,
        {
          method: "PATCH",
  ...
  async function removeTask(id: number) {
    try {
      await fetchJSON(`/guestmarkets/api/tasks/${id}`, { method: "DELETE" });
  ...
  async function toggleSubtask(s: Subtask) {
    ...
    try {
      const r = await fetchJSON<{ item: Subtask }>(
        `/guestmarkets/api/tasks/${sel.id}/subtasks/${s.id}`,
        {
          method: "PATCH",
              ...
              <div className="md:col-span-3">
                ...
                <div className="rounded-lg ring-1 ring-slate-200 p-2 grid grid-cols-1 sm:grid-cols-5 gap-2 bg-white">
                  ...
                  <div className="sm:col-span-1 text-[12px] text-slate-700 self-center">
                    {isRecurrent && freq !== "NONE" ? (
                      <>
                        FREQ <b>{freq}</b> • INTERVALLE <b>{interval}</b> • NBR{" "}
                        <b>{count}</b>
                      </>
                    ) : (
                      <>Pas de récurrence</>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          ...
          <section className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-3">
            ...
            {!loading && list.length === 0 && (
              <div className="text-slate-600 text-sm">Aucune tâche.</div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              value={sel.priority}
              onChange={(e) =>
                onChange({ ...sel, priority: e.target.value as Priority })
              }
          ...
          {/* Assigné à (lecture seule) */}
          <div className="md:col-span-2">
            <div className="text-[11px] text-slate-700 mb-1">Assigné à</div>
            <div className="text-sm text-slate-800">
              {(sel.assignees ?? []).length
                ? (sel.assignees ?? [])
                    .map((a) => a.name || a.email)
                    .join(", ")
                : "Vous"}
            ...
            <div className="text-[11px] text-slate-500 mt-1">
              Les tâches de cette vue sont automatiquement assignées à vous et
              ne peuvent pas être réattribuées.
            </div>
          </div>
                    ...
                    {s.description && (
                      <div
                        className={`text-xs mt-1 ${
                          s.done ? "text-slate-400" : "text-slate-600"
                        }`}
                      >
                        {s.description}
                      </div>
