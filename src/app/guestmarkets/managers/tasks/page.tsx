...
function AssigneePicker({
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const url = `/guestmarkets/api/scope/users${
  ...
  return (
    <div className="rounded-lg ring-1 ring-slate-200 p-2 bg-white">
      ...
      <div className="max-h-52 overflow-auto">
        {loading && (
          <div className="text-xs text-slate-600 px-1 py-1.5">Chargement…</div>
        )}
        {!loading && items.length === 0 && (
          <div className="text-xs text-slate-600 px-1 py-1.5">
            Aucun résultat
          </div>
        )}
...
export default function DepartmentTasksPage() {
  ...
  const [sort, setSort] = useState<"priority" | "date" | "name" | "progress">(
    "priority"
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        const me = await fetchJSON<{ ok: boolean; user?: { id: number } }>(
          `/guestmarkets/api/me`
        );
        if (!me.ok || !me.user) throw new Error("Non authentifié");

        const prof = await fetchJSON<{ department_id: number | null }>(
          `/guestmarkets/api/me/profile`
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await fetchJSON<{ items: Task[] }>(
          `/guestmarkets/api/admin/department/tasks?department_id=${departmentId}`
        );
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const data = await fetchJSON<{ items: Subtask[] }>(
          `/guestmarkets/api/admin/department/tasks/${sel.id}/subtasks`
        );
  ...
  const list = useMemo(() => {
    ...
    if (priorityFilter !== "all")
      arr = arr.filter((t) => getDisplayPriority(t) === priorityFilter);
    ...
    arr.sort((a, b) => {
      ...
      return (
        combineDateTime(a.due_date, a.due_time).getTime() -
        combineDateTime(b.due_date, b.due_time).getTime()
      );
    });
    return arr;
  ...
  function validateCreate(): string | null {
    ...
    if (d.getTime() < today.getTime())
      return "L’échéance ne peut pas être dans le passé.";

    if (dueTime) {
      if (!withinBusinessHours(dueTime))
        return "Choisis une heure entre 07:30 et 19:00.";
      const dt = combineDateTime(dueDate, dueTime);
      if (d.getTime() === today.getTime() && dt.getTime() < Date.now())
        return "Impossible de programmer avant l’heure actuelle.";
    }

    if (isRecurrent && freq !== "NONE") {
      if (interval < 1)
        return "L’intervalle de récurrence doit être au moins 1.";
      if (count < 1)
        return "Le nombre d’occurrences doit être au moins 1.";
    }
    return null;
  }
      ...
      const payload = {
        ...
        recurrence:
          isRecurrent && freq !== "NONE"
            ? { frequency: freq, interval, count }
            : { frequency: "NONE" },
        created_by: currentUserId,
        department_id: departmentId,
      };
      const out = await fetchJSON<{ item: Task }>(
        `/guestmarkets/api/admin/department/tasks`,
        {
          method: "POST",
  ...
  async function updateTask(patch: Partial<Task> & { id: number }) {
    ...
    try {
      const out = await fetchJSON<{ item: Task }>(
        `/guestmarkets/api/admin/department/tasks/${patch.id}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            ...patch,
  ...
  async function removeTask(id: number) {
    try {
      await fetchJSON(
        `/guestmarkets/api/admin/department/tasks/${id}`,
        { method: "DELETE" }
      );
  ...
  /* Sous-tâches (parent) */
  async function addSubtask(title: string, desc: string) {
    if (!sel || !title.trim()) return;
    try {
      const r = await fetchJSON<{ item: Subtask }>(
        `/guestmarkets/api/admin/department/tasks/${sel.id}/subtasks`,
        {
          method: "POST",
          body: JSON.stringify({
      ...
      const msg =
        e instanceof Error
          ? e.message
          : "Erreur lors de l'ajout de la sous-tâche.";
      setErr(msg);
    }
  }
  ...
  async function toggleSubtask(s: Subtask) {
    ...
    try {
      const r = await fetchJSON<{ item: Subtask }>(
        `/guestmarkets/api/admin/department/tasks/${sel.id}/subtasks/${s.id}`,
        {
          method: "PATCH",
      ...
      setSubtasks((prev) =>
        prev.map((x) => (x.id === s.id ? r.item : x))
      );
    } catch (e: unknown) {
      const msg =
        e instanceof Error
          ? e.message
          : "Erreur lors de la mise à jour de la sous-tâche.";
      setErr(msg);
    }
  }
  ...
  async function removeSubtask(s: Subtask) {
    ...
    try {
      ...
      const msg =
        e instanceof Error
          ? e.message
          : "Erreur lors de la suppression de la sous-tâche.";
      setErr(msg);
    }
  }
  ...
  return (
    <Shell
      ...
      sidebarSubtitle="Admin Département • Tâches"
    >
      {/* Fond doux */}
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          {formErr && (
            <div className="mb-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-1.5">
              {formErr}
            </div>
          )}
          <form onSubmit={createTask} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-2.5">
              ...
              <div className="md:col-span-3">
                <label className="block text-[11px] text-slate-700 mb-1 flex items-center gap-1">
                  <Users className="w-3.5 h-3.5" /> Attribuer à (périmètre
                  responsable)
                </label>
                <AssigneePicker
              ...
              <div className="md:col-span-3">
                <label className="block text-[11px] text-slate-700 mb-1 flex items-center gap-1">
                  <Repeat className="w-3.5 h-3.5" /> Récurrence (style Google
                  Agenda)
                </label>
                <div className="rounded-lg ring-1 ring-slate-200 p-2 grid grid-cols-1 sm:grid-cols-5 gap-2 bg-white">
                  ...
                  <div className="sm:col-span-1 text-[12px] text-slate-700 self-center">
                    {isRecurrent && freq !== "NONE" ? (
                      <>
                        FREQ <b>{freq}</b> • INTERVALLE <b>{interval}</b> •
                        NBR <b>{count}</b>
                      </>
                    ) : (
                      <>Pas de récurrence</>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                          ...
                          <div className="flex flex-wrap items-center gap-1 shrink-0">
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${prBadge}`}
                            >
                              Priorité{" "}
                              {dispPr === "high"
                                ? "Haute"
                            ...
                            <span
                              className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${statusBadge}`}
                            >
                              {statusText}
                            </span>
                          </div>
                        ...
                        <div className="text-[12px] text-slate-600 mt-1">
                          ...
                          {(t.assignees ?? []).length
                            ? (t.assignees ?? [])
                                ...
                                .join(", ")
                            : "—"}{" "}
                          • Progression : {t.progress}% • Performance :{" "}
                          {t.performance}%
                        </div>
                      </div>
...
function TaskDetails({
  ...
  subtasks: Subtask[];
  onAddSubtask: (title: string, desc: string) => void;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input
              value={sel.title}
              onChange={(e) =>
                onChange({ ...sel, title: e.target.value })
              }
              className="h-9 w-full px-3 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm text-slate-800 placeholder:text-slate-500 bg-white"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={todayStr()}
              onChange={(e) =>
                onChange({ ...sel, due_date: e.target.value })
              }
              className="h-9 w-full px-2 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm text-slate-800 placeholder:text-slate-500 bg-white"
            />
          </div>
          <div>
            ...
            <input
              ...
              step={60}
              onChange={(e) =>
                onChange({ ...sel, due_time: e.target.value || null })
              }
              className="h-9 w-full px-2 rounded-lg ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm text-slate-800 placeholder:text-slate-500 bg-white"
            />
          </div>
          ...
          <div className="md:col-span-2">
            <div className="text-[11px] text-slate-700 mb-1">
              Assignés (périmètre responsable)
            </div>
            <AssigneePicker
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) =>
                onChange({ ...sel, description: e.target.value })
              }
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  onChange({ ...sel, is_recurrent: e.target.checked })
                }
              />
          ...
          {([
            ...
            return (
              <button
                ...
                disabled={disabled}
                onClick={() =>
                  onSave({ ...sel, status: code as Status })
                }
                className={cls(
                  "px-3 py-2.5 rounded-lg text-sm font-medium ring-1 transition-all flex items-center gap-2 justify-center",
                  disabled
                    ? "opacity-50 cursor-not-allowed bg-slate-50 text-slate-400 ring-slate-200"
                    : active
                    ? activeC
                    : base
                )}
        ...
        <div className="pt-3 border-t border-slate-200">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {subtasks.map((s) => (
              <div
                key={s.id}
                className="p-2 rounded-lg ring-1 ring-slate-200 hover:ring-slate-300 transition-colors"
              >
