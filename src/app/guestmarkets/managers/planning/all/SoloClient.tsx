...
function PlanningUI() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Déterminer le département principal (mode des department_id non nuls)
  ...
  // Participants visibles selon périmètre (Département / Projet / Équipe)
  const participants = useMemo(() => {
    ...
    // fallback : si aucun résultat, on prend tout le périmètre
  ...
  async function createMeeting(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!validStart(start_at)) {
      alert("La réunion doit commencer dans ≥ 30 min");
      return;
    }
    ...
    <Shell sidebarTitle="Guest Office" sidebarSubtitle="Planning — Réunions">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="text-slate-700">
            Planifier et gérer vos réunions dans votre périmètre.
          </div>
          <button
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            list.map(({ ev, pct, pr, st, missed, urgent }) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[12px] text-slate-600">
                    {Number.isFinite(pct) ? pct : 0}% vers l’échéance
                  </div>
          ...
          {!loading && list.length === 0 && (
            <div className="col-span-full text-slate-500">Aucune réunion.</div>
          )}
        </section>
      </main>
      ...
      >
        <form onSubmit={createMeeting} className="space-y-4 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-500 mt-1">
                Elle s’ajustera automatiquement à 50% / 70% de l’échéance.
              </div>
            </div>
          </div>
          ...
          {/* Périmètre : Projets & Équipes */}
          <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
            <div className="text-[12px] text-slate-700 font-medium">
              Périmètre de sélection des participants
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label className="inline-flex items-center gap-2 text-[12px] text-slate-700">
                  ...
                  />
                  <span>Filtrer par équipe (facultatif)</span>
                </label>
                <select
            ...
            <div className="text-[11px] text-slate-500">
              Si aucun projet ni équipe n’est activé, les participants listés
              proviennent en priorité de votre département managé.
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-[12px] text-slate-600">
                Participants (dans votre périmètre)
              </div>
              <label className="inline-flex items-center gap-2 text-[12px] text-slate-700">
                <input
                  type="checkbox"
            ...
            <div className="max-h-56 overflow-auto rounded-xl ring-1 ring-slate-200 divide-y bg-white">
              ...
              {participants.length === 0 && (
                <div className="px-3 py-2 text-[12px] text-slate-500">
                  Aucun utilisateur disponible dans votre périmètre.
                </div>
              )}
            </div>
          </div>
