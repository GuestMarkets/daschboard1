...
export async function GET() {
  try {
    ...
    const hasCalendarEvents = await tableExists("calendar_events");
    const hasCalendarAttendees = await tableExists("calendar_event_attendees");

    // Si les tables n'existent pas, on renvoie simplement une liste vide,
    // sans faire planter le dashboard.
    if (!hasCalendarEvents) {
      return NextResponse.json({ items: [] as EventLite[] });
    }
    ...
    if (eventIds.length > 0 && hasCalendarAttendees) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!hasCalendarEvents || !hasCalendarAttendees) {
      return NextResponse.json(
        { error: "Module calendrier non configuré (tables manquantes)" },
        { status: 400 }
      );
    }
