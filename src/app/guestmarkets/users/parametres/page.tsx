...
// Icônes (uniquement celles utilisées)
...
export default function AccountSettingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const data = (await res.json()) as AccountLoadResponse;
        if (!res.ok) throw new Error(data?.error || "Impossible de charger le profil.");
        const m = data.me as Me;
        setMe(m);
        setName(m.name);
        setEmail(m.email);
        setRole(m.role);
        ...
        setLoadErr(message ?? "Erreur inconnue");
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      // data: { success, suspended?: boolean, message }
      ...
      const message = e instanceof Error ? e.message : String(e);
      setSaveErr(message ?? "Erreur lors de l’enregistrement.");
    } finally {
      setSaving(false);
    }
  }
              ...
              <Card className="p-6 space-y-4">
                ...
                {!isAdmin && (
                  <label className="flex items-start gap-2 text-[13px] text-slate-700">
                    ...
                    />
                    <span>
                      Je comprends qu’en enregistrant ces modifications, <b>mon compte sera suspendu</b> jusqu’à vérification par un administrateur.
                    </span>
                  </label>
                )}
