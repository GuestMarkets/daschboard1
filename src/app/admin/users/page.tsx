...
type UsersStreamEvent = { kind: "reload" | "userUpdated" };

/* ---------------- Constantes stables (pour hooks) ---------------- */
const ALL_SCOPES = [
  "tasks",
  "meetings",
  "objectives",
...
type ReassignScope = (typeof ALL_SCOPES)[number];
...
function isUsersStreamEvent(v: unknown): v is UsersStreamEvent {
  return (
    isRecord(v) &&
    typeof v.kind === "string" &&
    (v.kind === "reload" || v.kind === "userUpdated")
  );
}
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (res && typeof res === "object" && "counts" in res) {
                    const r = res as { counts?: Record<string, number> };
...
export default function UsersAdminPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const onUsers = (e: MessageEvent<string>) => {
        try {
          const parsed: unknown = JSON.parse(e.data || "{}");
          if (isUsersStreamEvent(parsed)) {
            if (parsed.kind === "reload" || parsed.kind === "userUpdated") {
              loadUsers();
            }
          }
        } catch {
      ...
      // cast léger pour satisfaire le type EventListener
      es.addEventListener("users", ((evt: Event) =>
        onUsers(evt as MessageEvent<string>)) as EventListener);
      es.onerror = () => {
        /* silencieux */
      };
    } catch {
    ...
    if (catFilter !== "all") {
      const m =
        catFilter === "gm"
          ...
          ? "Guest Cameroon"
          : "Autre";
      arr = arr.filter((u) => (u as UserWithCat).category === m);
    }
    return arr as UserWithCat[];
  ...
  return (
    <Shell sidebarTitle="Guest Office" sidebarSubtitle="Utilisateurs">
      {/* En-tête + stats */}
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-3">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          ...
          <div className="rounded-xl ring-1 ring-blue-200 bg-blue-50 px-3 py-2">
            <div className="text-[11px] text-blue-700">G.Mark</div>
          ...
          <div className="rounded-xl ring-1 ring-indigo-200 bg-indigo-50 px-3 py-2">
            <div className="text-[11px] text-indigo-700">G.Cam</div>
          ...
          <div className="rounded-xl ring-1 ring-teal-200 bg-teal-50 px-3 py-2">
            <div className="text-[11px] text-teal-700">Resp</div>
          ...
          <div className="rounded-xl ring-1 ring-amber-200 bg-amber-50 px-3 py-2">
            <div className="text-[11px] text-amber-700">Susp</div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-2.5 sm:p-3 mt-2 mb-2">
