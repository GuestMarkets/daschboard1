...
const composeSQLDatetime = (ymd:string, hm:string)=>{
  const [y,m,d]=ymd.split("-").map(Number); const [hh,mm]=hm.split(":").map(Number);
...
function SoloUI(){
  ...
  const refresh = useCallback(async ()=>{
    try {
      ...
      setErr(getErrorMessage(e, "Erreur rafraîchissement"));
    }
  },[]);
  ...
  useEffect(()=> {
    (async()=>{
      try{
        ...
        setItems(ev.items||[]);
        setErr(null);
      }catch(e: unknown){ setErr(getErrorMessage(e, "Erreur de chargement")); }
      finally{ setLoading(false); }
    })();

    const unsub = rt.subscribe((a: RealtimeEvent) => { if (a.type==="meetings_changed") refresh(); });
    return unsub;
  }, [rt, refresh]);
