...
/** Schémas “bruts” acceptés depuis l’API (clés alternatives, types souples) */
type RawSubtask = {
  id?: number | string;
...
async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  let parsed: unknown = {};
  if (txt) {
    try { parsed = JSON.parse(txt) as unknown; }
    catch { parsed = { error: txt.slice(0, 200) } as ApiErrorShape; }
  }

  if (!res.ok) {
    const reason = (parsed as ApiErrorShape)?.error ?? `HTTP ${res.status}`;
...
export default function MyObjectivesPage() {
  ...
  function normalize(o: RawObjective): Objective {
    return {
      ...
      subtasks: Array.isArray(o.subtasks)
        ? o.subtasks.map((s: RawSubtask) => ({
            id: Number(s.id ?? 0),
            objective_id: Number(s.objective_id ?? o.id ?? 0),
            title: String(s.title ?? ""),
            weight: Number(s.weight ?? 0),
            dueDate: s.dueDate ?? s.due_date ?? null,
            done: (s.done === true ? 1 : s.done === 1 ? 1 : 0) as 0 | 1,
          }))
        : [],
    };
  }

  /* --------- Progression & priorité affichée (mémoïsées) --------- */
  const computeProgress = useCallback((o: Objective) => {
  ...
  const displayPriority = useCallback(
    (o: Objective): Priority => autoPriority(o.priority, computeProgress(o)),
    [computeProgress]
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const o = await fetchJSON<{ items: RawObjective[] }>("/api/objectives?scope=my");
  ...
  /* --------- Type guards pour les <select> --------- */
  const isObjStatus = (v: string): v is ObjStatus =>
    v === "todo" || v === "in_progress" || v === "done";

  const isSortKey = (v: string): v is "priority" | "end" | "progress" =>
    v === "priority" || v === "end" || v === "progress";
  ...
  const list = useMemo(() => {
    ...
    return arr;
  }, [items, q, statusFilter, sort, computeProgress, displayPriority]);
  ...
  async function createObjective(e: React.FormEvent) {
    ...
    try {
      ...
      const out = await fetchJSON<{ item: RawObjective }>("/api/objectives", {
        method: "POST",
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            ...
            const Icon = k.icon as React.ComponentType<{ className?: string }>;
            return (
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                  ...
                  if (v === "all" || isObjStatus(v)) setStatusFilter(v);
                }}
...
function EditObjectiveModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const r = await fetchJSON<{ item: RawObjective }>(`/api/objectives/${local.id}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      });
