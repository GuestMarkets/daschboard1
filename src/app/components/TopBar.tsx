  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 md:gap-2.5 z-10">
        {mounted && (
          <div
            className="hidden sm:flex items-center gap-2 pl-2 pr-2.5 py-1.5 rounded-lg bg-white ring-1 ring-slate-200 backdrop-blur-sm"
            title="Date & heure"
          >
            <CalendarDays className="w-4 h-4 opacity-80 text-slate-600" />
            <span className="tabular-nums text-sm text-slate-800">{dateStr}</span>
            <span className="opacity-40 text-slate-400">•</span>
            <Clock className="w-4 h-4 opacity-80 text-slate-600" />
            <span className="tabular-nums text-sm text-slate-800">{timeStr}</span>
          </div>
        )}
